...
select count("name") AS "USER 資料表目前所有用戶數量"
from "USER"; 
...
values
	( 
		...
		(select P.id from "CREDIT_PACKAGE" P where P.name = '14 堂組合包方案' ),
		(select P.credit_amount from "CREDIT_PACKAGE" P where P.name = '14 堂組合包方案' ),
		(select P.price from "CREDIT_PACKAGE" P where P.name = '14 堂組合包方案' )
	),
	( 
		(select U.id from "USER" U where U.name = '王小明'), 
		(select P.id from "CREDIT_PACKAGE" P where P.name = '21 堂組合包方案' ),
		(select P.credit_amount from "CREDIT_PACKAGE" P where P.name = '21 堂組合包方案' ),
		(select P.price from "CREDIT_PACKAGE" P where P.name = '21 堂組合包方案' )
	),
	( 
